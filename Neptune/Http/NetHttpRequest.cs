...
namespace Neptune.Http
{
	public class NetHttpRequest : HttpRequest
	{
		...
		protected override void InternalAbort()
		{
				...
				if (thread != null)
				{
					if (isThreadAlive)
					{
						if (isThreadAlive)
						{
							try
							{
								Log.Logger.Log("***************[NetHttpRequest.InternalAbort] Thread State = " +
								               thread.ThreadState);
								thread.Abort();
							}
							catch (Exception ex)
			...
			{
				Log.Logger.Error("[NetHttpRequest.InternalAbort] error = " + ex2.Message);
				Log.Logger.Error("[NetHttpRequest.InternalAbort] error stack trace = " + ex2.StackTrace);
			}
		}
			...
			try
			{
				...
				if (a == "gzip")
				{
					using (ResponseStream responseStream = new ResponseStream(httpWebResponse.GetResponseStream()))
					{
						...
						{
							Log.Logger.Log("[HttpRequest] start uncompress gzip by SharpZipLib");
